...
class Publication(TimestampMixin, db.Model):
    ...
    author_type = db.Column(db.String(32))  # First Author, Co-author
    journal_type = db.Column(db.String(16))  # Q1-Q4 tier info
...
class ApplicationRecord(TimestampMixin, db.Model):
    ...
    def to_public_dict(self) -> Dict[str, Any]:
        gpa, gpa_scale = get_effective_gpa(self.user)
        ...
        return {
            ...
            "gpa": gpa,
            "gpa_scale": gpa_scale,
            "gre_total": profile.gre_total if profile else None,
            "research_experience": profile.research_experience if profile else False,
            "internship_experience": profile.internship_experience if profile else False,
            "recommendation_strength": profile.recommendation_strength if profile else None,
            "created_at": self.created_at.isoformat() if self.created_at else None,
        }
...
def normalize_publication_payload(payload: Dict[str, Any]) -> None:
    if "author_type" in payload and "first_author" not in payload:
        payload["first_author"] = payload["author_type"] == "First Author"
    if "first_author" in payload and "author_type" not in payload:
        payload["author_type"] = "First Author" if payload["first_author"] else "Co-author"


def select_primary_education(user: Optional[User]) -> Optional[Education]:
    if not user or not user.educations:
        return None

    def priority(entry: Education) -> Tuple[int, int, int, int]:
        degree = (entry.degree or "").lower()
        preferred_keywords = ("under", "bachelor", "bs", "b.s")
        rank = 0 if any(keyword in degree for keyword in preferred_keywords) else 1
        return (
            rank,
    ...
    return sorted(user.educations, key=priority)[0]


def get_effective_gpa(user: Optional[User]) -> Tuple[Optional[float], Optional[float]]:
    if not user:
        return None, None
    profile = user.profile
    if profile and profile.gpa and profile.gpa_scale:
        return profile.gpa, profile.gpa_scale

    primary = select_primary_education(user)
    if primary and primary.gpa and primary.gpa_scale:
        return primary.gpa, primary.gpa_scale
    return None, None
...
def register_routes(app: Flask) -> None:
    ...
    def publication_collection():
        ...
        fields = ["title", "venue", "year", "first_author", "author_type", "journal_type", "url"]
    ...
    def create_application():
        ...
        assert user
        payload = request.get_json(silent=True) or {}
        if not payload.get("university") or not payload.get("program") or not payload.get("result") or not payload.get("term"):
            return jsonify({"error": "university, program, term, and result are required"}), 400
    ...
    def match_suggestions():
        user = get_current_user()
        assert user
        gpa, gpa_scale = get_effective_gpa(user)
        if not gpa or not gpa_scale:
            return (
                jsonify({"error": "Add an education record with GPA to receive match suggestions."}),
                400,
            )

        user_gpa_norm = gpa / gpa_scale
        profile = user.profile
        user_gre = (profile.gre_total if profile and profile.gre_total else 0)
...
def aggregate_program_stats() -> Dict[str, Dict[str, Any]]:
    ...
    for record in records:
        ...
        normalized_gpa = None
        gpa, gpa_scale = get_effective_gpa(record.user)
        if gpa and gpa_scale:
            normalized_gpa = gpa / gpa_scale

        if record.result and record.result.lower() == "accept":
            entry["admitted"] += 1
            if normalized_gpa is not None:
                entry["gpa_values"].append(normalized_gpa)
            profile = record.user.profile if record.user else None
            if profile and profile.gre_total:
                entry["gre_values"].append(profile.gre_total)
