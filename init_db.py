...
Generates several hundred application records plus a ready-to-use demo account.
"""
...
DEFAULT_USER_PASSWORD = "applyCS!2024"
DEMO_ACCOUNT_EMAIL = "demo_account@example.com"
DEMO_ACCOUNT_PASSWORD = "demo_pass123"
...
def create_sample_data():
    ...
    with app.app_context():
        ...
        # Generate several hundred application records with associated user data
        num_users = 300
        ...
        for user_idx in range(num_users):
            ...
            if random.random() > 0.6:  # 40% have publications
                ...
                for pub_idx in range(num_pubs):
                    is_first_author = random.random() > 0.5
                    pub = Publication(
                        user_id=user.id,
                        ...
                        author_type="First Author" if is_first_author else "Co-author",
                        journal_type=random.choice(["Q1", "Q2", "Q3", "Q4"]),
        ...
        demo_apps = create_demo_account()
        applications_created += demo_apps
...
def create_demo_account() -> int:
    """Create a ready-to-use demo account with pre-filled data."""
    existing = User.query.filter_by(email=DEMO_ACCOUNT_EMAIL).first()
    if existing:
        print("Demo account already exists; skipping creation.")
        return 0

    user = User(
        email=DEMO_ACCOUNT_EMAIL,
        password_hash=generate_password_hash(DEMO_ACCOUNT_PASSWORD),
    )
    db.session.add(user)
    db.session.flush()
    ...
    publication = Publication(
        user_id=user.id,
        title="Demo Paper: Efficient Graph Neural Networks",
        venue="NeurIPS",
        year=2023,
        first_author=True,
    ...
    experience = Experience(
        user_id=user.id,
        category="Research",
        organization="UW NLP Lab",
        role_title="Research Assistant",
        description="Worked on large language model pretraining.",
        start_date="2021-06",
        end_date="2022-06",
        ongoing=False,
    )
    ...
    db.session.commit()
    print(
        f"Created demo account: {DEMO_ACCOUNT_EMAIL} / {DEMO_ACCOUNT_PASSWORD} (3 application records)"
    )
